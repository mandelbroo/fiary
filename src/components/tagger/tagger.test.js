...
describe('Tagger', () => {
  it('contain input', () => {
    ...
    expect(wrapper.containsAllMatchingElements([<input type="text" />])).toBe(
      true
    )
  })
  ...
  it('call fake tag service', async () => {
    ...
    const fakeTagService = {
      find: jest
        .fn()
        .mockImplementation(() => Promise.resolve({ data: fakeTags })),
    }
    const fakeChange = jest.fn()
    const wrapper = shallow(
      <Tagger service={fakeTagService} onChange={fakeChange} />
    )
