...
describe('Signup', () => {
	it('successful sign up', async () => {
		...
		pass.first().simulate('change', { target: { name: 'password', value: 'secret' } })
		pass.last().simulate('change', { target: { name: 'password', value: 'secret' } })
		wrapper.find('form').simulate('submit', { preventDefault: () => {} })
		await promise
		expect(fakeHistory.push).toBeCalledWith('/')
		const params = {email: 'some@mail', password: 'secret', username: 'someone'}
	...
	it('unsuccessful sign up', async () => {
		...
		const fakeUser = {
			signup: jest.fn().mockImplementation(() =>
				promise = Promise.resolve({success: false, message: 'some error'}))
		}
		const fakeHistory = { push: jest.fn() }
		const wrapper = mount(<Signup user={fakeUser} history={fakeHistory}/>)
		wrapper.find('[type="email"]')
